...
public class GameSessionController {
    ...
    public void createGameSession(@RequestBody GameSession session) {
        if (session.getSessionId() != 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"sessionId is autogenerated and cannot be set manually.");
        }

        if (session.getDate() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Game session must have a date.");
        }

        if (session.getGame() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Game session must have a game.");
        }

        gameSessionService.createGameSession(session);
    }
